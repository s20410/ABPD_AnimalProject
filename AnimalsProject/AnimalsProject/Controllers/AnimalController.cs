...
namespace AnimalsProject.Controllers
{
    ...
    public class AnimalController : ControllerBase
    {
        ...
        [HttpPut( "{idAnimal}" )]
        public Task<IActionResult> UpdateAnimal ( Animal animal, int idAnimal )
        {
            var result = _animalService.UpdateAnimal( animal, idAnimal );
            if ( result == 1 )
            {
                return Task.FromResult<IActionResult>(Ok( "Animal updated" ));
            }
            return Task.FromResult<IActionResult>(NotFound("The animal has not been updated"));
        }
        [HttpDelete("{idAnimal}")]
